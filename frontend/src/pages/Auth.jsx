...
import { Search, ShoppingCart, Trash2, X, Menu, LogOut } from 'lucide-react';
...
const Auth = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await mockApi.post(endpoint, formData);
      
      if (res.data.token) {
        // Store token in memory (not localStorage due to artifact restrictions)
        const userData = {
        ...
        alert('Success! Redirecting to chat...');
      } else {
...
// Navbar Component
const Navbar = ({ cartCount, onMenuToggle, isMenuOpen, user, onLogout }) => (
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white/10 backdrop-blur-sm border-t border-white/20">
          <div className="px-4 py-4 space-y-3">
            ...
            <button 
              onClick={onLogout}
              className="w-full text-left py-2 hover:bg-white/10 rounded flex items-center space-x-2"
            >
              <LogOut className="h-4 w-4" />
              <span>Logout</span>
            </button>
            ...
            <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-4 text-white">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-indigo-100 text-sm">Welcome back, {user?.email}!</p>
                </div>
                <div className="flex items-center space-x-2">
